...
CREATE TABLE `t_equipment` (
  ...
  `updated` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `version` int(11) NOT NULL DEFAULT '0',
  UNIQUE KEY `equipment_brand_model_UNIQUE` (`brand_id`, `model`),
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8;
...
CREATE TABLE `t_product` (
  ...
  `name_en_customized` bit not null,
  ...
  `description_en_customized` bit not null,
  `name_zh` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT '',
  `name_zh_customized` bit not null,
  ...
  `description_zh_customized` bit not null,
