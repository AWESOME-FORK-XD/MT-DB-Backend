/* 
  Adds European and Chinese prices to product table.

  Adds `list_price_eu` and `list_price_zh` to `t_product`
...
-- Add columns to t_product
alter table t_product add column list_price_eu decimal(9,2) not null;
alter table t_product add column list_price_zh decimal(9,2) not null;

-- ...these columns should also be included on views related to t_product. Drop and re-add as necessary.
...
-- Also include v_product_catalog which includes includes references to v_product!
...
-- Don't forget to update the master db-create.sql and db-views-create.sql to incorporate these changes!